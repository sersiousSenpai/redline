...
| `polis-store` | A2–A3 | rusqlite store: attach to a host connection or open standalone (WAL), its own migrations under `polis_meta` (never `PRAGMA user_version`), the lexical layer, the ~120 memory methods, recorders, compaction/archive, backup | `polis-core`, `rusqlite` (bundled), `flate2`, `regex-lite` |
