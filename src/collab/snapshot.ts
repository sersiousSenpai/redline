...
/** One revision's metadata for the viewer's history timeline (no body). */
export interface SnapRevisionMeta {
  version: number;
  createdAt: number;
  title?: string;
  threadStart?: boolean;
...
/** A prior comment/discussion thread, carried read-only into the viewer.
 *  `author` is the human reviewer name only (never an agent id). */
...
/** A resolved decision (sourced from resolved/accepted comments). */
export interface SnapDecision {
  title: string;
  decidedAt: number;
  disposition: "resolved" | "accepted" | "decision";
}

/** Derived plan stats for the viewer's hero strip. */
...
/** One heading in the pruned table of contents. */
export interface SnapTocNode {
  title: string;
  level: number;
  anchorId: string;
  blockId: string;
...
export interface SnapshotPayload {
  ...
  // ── enrichment (all optional; absent on old links / when toggled off) ──
  /** Every revision's metadata, so the viewer can show the plan's history. */
  revisionTimeline?: SnapRevisionMeta[];
  /** Prior discussion carried read-only (gated by the sharer). */
  discussion?: SnapComment[];
  /** Resolved decisions digest (gated by the sharer). */
  decisions?: SnapDecision[];
  /** Derived counts + reading time. */
  stats?: SnapStats;
  /** Pruned heading-only table of contents (never carries block bodies). */
  toc?: SnapTocNode[];
}
